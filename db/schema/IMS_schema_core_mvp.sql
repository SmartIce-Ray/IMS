...
-- 使用唯一索引代替约束（支持COALESCE表达式）
CREATE UNIQUE INDEX uk_unit_conversion ON unit_conversion(from_unit_id, to_unit_id, COALESCE(product_id, 0));
...
CREATE UNIQUE INDEX uk_recipe_version ON recipe(product_id, version, COALESCE(brand_id, 0));
...
CREATE UNIQUE INDEX uk_inventory ON inventory(warehouse_id, product_id, COALESCE(batch_number, ''));
...
CREATE TABLE sales_detail (
    ...
    -- ★计算字段（由触发器自动更新）
    theoretical_cost DECIMAL(10,2) DEFAULT 0,      -- 理论成本
    standard_cost_rate DECIMAL(5,2) DEFAULT 0,     -- 标准成本率
    actual_cost_rate DECIMAL(5,2) DEFAULT 0,       -- 实际成本率
    cost_variance DECIMAL(5,2) DEFAULT 0,          -- 成本率差异
    gross_margin_rate DECIMAL(5,2) DEFAULT 0,      -- 毛利率
...
CREATE UNIQUE INDEX uk_sales_summary ON sales_summary(summary_period, COALESCE(store_id, 0), product_id);
...
CREATE UNIQUE INDEX uk_profit_report ON profit_report(report_period, COALESCE(store_id, 0), COALESCE(brand_id, 0));
